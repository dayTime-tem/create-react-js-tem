 ...
 */
const Img = (props) => {
    const { data, preview = true, width = 105, height, del } = props
    return (
        <div>
            <Image.PreviewGroup>
                {data.map(v => (
                    <div className={classNames(style.img_container)} key={v.url} style={{width, height: height || width}}>
                        <div className={classNames(style.img_container_wrapper)}>
                            <Image
                                ...
                                fallback={fallback}
                                preview={preview}
                                placeholder={<div className={classNames(style.loading)}><Spin /></div>}
                            />
                            { del && <div className={classNames(style.img_handle)} onClick={() => del(v)}><CloseOutlined /></div>}
                        </div>
                    </div>
                ))}
            </Image.PreviewGroup>
